...
MASTI_CHANNEL_ID        = os.getenv("MASTI_CHANNEL_ID")          # 🔥 канал прогнозов мастей
...
SERIES_TRIGGERS= set(int(x) for x in os.getenv("SERIES_TRIGGERS", "2,4").split(","))
...
# 🔥 Масти по mod 4 (последняя цифра ID % 4)
SUITS_MAP = {
...
mast_preds = {}  #  {display_id: {"msg_id": ..., "suit_code": ..., "first_n": ..., "second_n": ...}}
...
def get_suit_by_id(game_id):
    """Определяет масть по последней цифре длинного ID (mod 3)"""
    last_digit = int(str(game_id)[-1])
    
    # Особая логика: если цифра кратна 3 (3, 6, 9) → червы
...
def parse_player_suit(text):
    """Парсит масть игрока (первая скобка) из текста статистики"""
    groups = re.findall(r'\(([^)]*)\)', text or "")
    if len(groups) < 1: return None
    cards = groups[0]
    suits = SUITS_RE.findall(cards)
...
def finalize_mast(display_id, success, detail):
    """Обновляет прогноз масти"""
    if display_id not in mast_preds: return
    pred = mast_preds[display_id]
    try:
        if success:
            text = (f"🎯 Игра #N{display_id}\n"
                    f"Игрок {pred['suit_symbol']} {success}")
        else:
            text = (f"🎯 Игра #N{display_id}\n"
                    f"Игрок {pred['suit_symbol']} ❌")
        bot.edit_message_text(
            chat_id=MASTI_CHANNEL_ID, message_id=pred["msg_id"],
            text=text)
...
def create_mast_prediction(game_id, display_id):
    """Создаёт прогноз масти для игры"""
    ...
    if sent:
        ...
        print(f"🎯 МАСТЬ #N{display_id} → {suit_info['symbol']} (ID {game_id} mod4={suit_code})")
    ...
    with lock:
        for pred in list(active_preds):
            if num in {pred["first_n"], pred["second_n"]} and num not in pred["checked"]:
                ...
                if has_nat:
                    pos = "0️⃣" if num == pred["first_n"] else "1️"
    ...
    if num in mast_preds:
        ...
        if actual_suit:
            if actual_suit == pred["suit_symbol"]:
                ...
                finalize_mast(num, f"✅{pos}", None)
                print(f"🎉 МАСТЬ УГАДАНА #N{num} ({pos})")
            else:
                # Не угадали на этом номере, но может угадать на следующем
                pred["checked"].add(num)
                if len(pred["checked"]) >= 2:
                    finalize_mast(num, False, None)
...
def api_cycle():
    ...
    with lock:
        ...
        for display_id, pred in list(mast_preds.items()):
            if time.time() - pred["created_at"] > PRED_TIMEOUT:
                finalize_mast(display_id, False, None)
    ...
    for game in games:
        ...
        if di: 
            ...
            # 🔥 Создаём прогноз масти для каждой новой игры
